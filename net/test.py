...
output_dir=abs+f'pred_FD_CA_{dataset}/'
print("pred_dir:",output_dir)
if not os.path.exists(output_dir):
    os.mkdir(output_dir)
model_dir=abs+f'trained_models/{dataset}_train_fdca_{gps}_{blocks}.pk'
device='cuda' if torch.cuda.is_available() else 'cpu'
ckp=torch.load(model_dir,map_location=device)
net=FD_CA(gps=gps,blocks=blocks)
...
for im in os.listdir(img_dir):
    ...
    vutils.save_image(ts,output_dir+im.split('.')[0]+'_FD_CA.png')
